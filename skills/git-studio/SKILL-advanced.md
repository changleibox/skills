...
> 暂存当前工作区变更：
> [列出变更文件]
> - **Stash 描述信息**：（可选，便于后续识别）
> - **包含范围**：
>   1. **仅已跟踪文件**（默认）
>   2. **含未跟踪文件** (`-u`)
>   3. **含所有文件**（包括 .gitignore 忽略的）
...
> ⚠️ 恢复 stash 时产生冲突！
> 1. **解决冲突** — 手动编辑冲突文件
> 2. **恢复到新分支** — `git stash branch <new-branch>`
> 3. **放弃恢复** — stash 仍保留在列表中
...
> 重置到目标：`<commit>` (`<commit message>`)
> 请选择重置模式：
> 1. **Soft** — 仅移动 HEAD，保留暂存区和工作区（安全）
> 2. **Mixed** — 移动 HEAD + 清空暂存区，保留工作区（安全）
> 3. **Hard** — 全部重置，丢弃所有变更（⚠️ 危险）
> 4. **取消**

**选择 Hard 模式时的二次确认：**
> ⛔ **警告：Hard Reset 会丢失所有未提交的修改！**
> 以下变更将被永久丢弃：
> [列出将被丢弃的文件变更]
> 1. **确认 Hard Reset** — 我了解风险
> 2. **改用 Soft/Mixed** — 保留代码
> 3. **取消**

**Reset 已推送提交的警告：**
> ⚠️ 目标提交已推送到远程！
> Reset 后需要 force push，可能影响其他协作者。
> 1. **继续 Reset** — 这是我的个人分支
> 2. **改用 Revert** — 更安全，不改写历史
> 3. **取消**
...
> 撤销提交 `<commit-hash>`: `<commit message>`
> 这将创建一个新的提交来反向撤销该提交的变更。
> 1. **Revert 并自动提交** — 直接生成撤销提交
> 2. **Revert 不提交** — 仅应用变更到工作区，稍后手动提交
> 3. **取消**

**撤销合并提交时的额外询问：**
> ⚠️ 这是一个合并提交，需要指定保留哪个父提交：
> 1. **保留主分支** (parent 1) — 撤销合并进来的变更（最常见）
> 2. **保留合入分支** (parent 2) — 撤销主分支的变更（罕见）
...
> - 目标提交：`<commit>` (`<message>`)
> - 是否已推送到远程：[是/否]
> 1. **Revert**（推荐）— 创建新提交撤销，安全不改写历史
> 2. **Reset Soft** — 回退但保留代码在暂存区
> 3. **Reset Hard** — 彻底回退，丢弃所有变更（⚠️ 危险）
...
> ⚠️ 删除后不可恢复！
> 请选择删除范围：
> 1. **仅未跟踪文件** (`git clean -f`)
> 2. **未跟踪文件+目录** (`git clean -fd`)
> 3. **含 .gitignore 忽略的文件** (`git clean -fdx`) — ⛔ 极危险
> 4. **取消**
...
**🔄 当用户想找回丢失的提交/分支时询问：**
> 想要找回什么？
> 1. **误操作的 reset --hard** — 从 reflog 找回丢弃的提交
> 2. **被删除的分支** — 找到分支最后的提交并重建
> 3. **被 rebase 覆盖的提交** — 找到 rebase 前的状态
> 4. **查看操作历史** — 仅查看 reflog
...
> - **补丁来源**：
>   1. **工作区变更**（未提交的修改）
>   2. **指定提交范围** — 从 `<commit1>` 到 `<commit2>`
>   3. **指定文件的变更**
>   4. **取消**
> - **保存路径**：（默认：`./changes.patch`）
...
> - **补丁文件路径**：
> - **应用方式**：
>   1. **直接应用**（apply）— 仅修改文件，不创建提交
>   2. **应用并提交**（am）— 应用 format-patch 生成的补丁，保留提交信息
>   3. **预览**（dry-run）— 仅检查能否应用
>   4. **取消**
...
> GPG 签名配置：
> - **当前状态**：[agent 读取 `git config user.signingkey` 展示]
> 1. **选择已有 GPG Key** — 从已有密钥中选择
> 2. **查看可用密钥列表** — 列出所有 GPG 密钥
> 3. **开启/关闭自动签名** — 所有提交和标签自动签名
> 4. **跳过** — 不配置签名
...
**🔄 询问用户（当 hook 失败时）：**
> ⚠️ pre-commit hook 执行失败，提交被拒绝。
> [展示 hook 输出/错误信息]
> 1. **修复问题后重新提交**（推荐）
> 2. **跳过 hook 提交** — `git commit --no-verify`
> 3. **取消提交**
...
> SSH 配置：
> - **当前 SSH Key**：[agent 读取 `ls ~/.ssh/` 展示]
> - **SSH 连接状态**：[agent 测试 `ssh -T git@github.com` 展示]
> 1. **生成新 SSH Key** — 创建 ed25519 密钥对
> 2. **添加到 ssh-agent** — 将已有密钥加载到 agent
> 3. **测试连接** — 验证 SSH 连接到 GitHub/Gitee
> 4. **查看公钥** — 复制到代码托管平台
> 5. **跳过** — 不修改 SSH 配置
...
> 凭据存储方式：
> - **当前配置**：[agent 读取 `git config credential.helper` 展示]
> 1. **macOS 钥匙串** — `osxkeychain`（推荐 macOS 用户）
> 2. **内存缓存** — `cache`（默认15分钟过期）
> 3. **内存缓存（自定义时长）** — 指定缓存秒数
> 4. **永久存储** — `store`（明文保存，⚠️ 不推荐）
> 5. **清除已缓存凭据** — 清空当前缓存
> 6. **跳过** — 不修改凭据配置
...
**🔄 询问用户：**
> 远程仓库协议切换：
> - **当前远程 URL**：[agent 读取 `git remote -v` 展示]
> 1. **切换到 SSH** — 使用 SSH 密钥认证
> 2. **切换到 HTTPS** — 使用用户名/密码认证
> 3. **保持不变**
...
> | # | 配置项 | 当前值 |
> |---|--------|--------|
> | 1 | **用户名** (user.name) | [agent 读取展示] |
> | 2 | **邮箱** (user.email) | [agent 读取展示] |
> | 3 | **默认分支名** (init.defaultBranch) | [agent 读取展示] |
> | 4 | **默认编辑器** (core.editor) | [agent 读取展示] |
> | 5 | **换行符处理** (core.autocrlf) | [agent 读取展示] |
> | 6 | **Diff 工具** (diff.tool) | [agent 读取展示] |
> | 7 | **Merge 工具** (merge.tool) | [agent 读取展示] |
> | 8 | **Pull 策略** (pull.rebase) | [agent 读取展示] |
