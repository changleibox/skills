...
> 将 `<source-branch>` 合并到当前分支 `<current-branch>`，请选择合并策略：
> 1. **Merge** — 创建合并提交，保留分支拓扑（推荐）
> 2. **Squash and Merge** — 压缩为单个提交，历史更干净
> 3. **Rebase** — 变基合并，保持线性历史
> 4. **取消**
...
2. **🔄 询问用户（逐个点选排序）：**
   > 以下是最近 5 个提交，请逐个点选确定新顺序（点选顺序 = 最终排序）：
   >
   > | # | 提交 | 信息 |
   > |---|--------|------|
   > | 1 | `abc1234` | feat: 用户登录 |
   > | 2 | `def5678` | fix: 修复样式 |
   > | 3 | `ghi9012` | feat: 注册页面 |
   > | 4 | `jkl3456` | refactor: 抽取工具类 |
   > | 5 | `mno7890` | docs: 更新 README |
   >
   > 请按你希望的顺序点选编号（如输入 `3` `1` `4` `2` `5`）：

   用户每次点选一个编号，agent 实时更新剩余列表：
   > ✅ 已选：3 → 1
   > 剩余可选：2, 4, 5
   > 请继续点选：

   直到所有提交都被选完，或用户输入 `done` 将剩余提交按原序追加。
...
> 你想要将分支 `<source>` 的变更合入 `<target>`，请选择方式：
> 1. **Rebase** — 保持线性历史，适合同步主分支更新
> 2. **Merge** — 保留分支拓扑，适合特性分支合回主分支
> 3. **Squash Merge** — 压缩为单提交，适合清理历史
...
> ⛔ **警告：分支 `<branch>` 已推送到远程！**
> Rebase 会改写提交历史，完成后需要 force push。
> 如果其他人也在使用此分支，会导致他们的本地仓库与远程不一致。
> 1. **继续 Rebase** — 这是我的个人分支，没关系
> 2. **改用 Merge** — 更安全的方式
> 3. **取消**
...
> 拣选提交到当前分支 `<current-branch>`：
> [列出选中的提交信息]
> 请确认：
> 1. **Cherry-pick 并自动提交** — 直接应用并生成新提交
> 2. **Cherry-pick 不提交** — 仅应用变更到暂存区，稍后手动提交
> 3. **取消**
...
> [列出冲突文件]
> 请选择解决方式：
> 1. **逐文件解决**（推荐）— 打开每个文件，AI 分析并建议合并方案
> 2. **全部接受当前分支** (Accept Ours) — 丢弃传入的变更
> 3. **全部接受传入分支** (Accept Theirs) — 丢弃当前的变更
> 4. **放弃操作** — abort 回到操作前状态

**对单个文件的冲突，询问用户（对应 AS 的 Merge Tool 三向合并）：**
> 文件 `<filename>` 冲突：
> [展示冲突内容摘要]
> 1. **AI 智能合并** — 分析双方意图，生成合并结果
> 2. **接受当前分支** (Accept Ours)
> 3. **接受传入分支** (Accept Theirs)
> 4. **跳过此文件** — 稍后手动处理
