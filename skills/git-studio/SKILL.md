...
> **凡是能列出选项的场景，必须以编号列表形式展示并让用户选择，禁止要求用户手动输入名称。**
>
> 具体要求：
> 1. **分支选择** — 先执行 `git branch -a` 获取分支列表，以编号列表展示，让用户选择编号
> 2. **标签选择** — 先执行 `git tag` 获取标签列表，以编号列表展示，让用户选择编号
> 3. **Stash 选择** — 先执行 `git stash list` 获取列表，以编号列表展示，让用户选择编号
> 4. **提交选择** — 先执行 `git log --oneline` 获取提交列表，以编号列表展示，让用户选择编号
> 5. **文件选择** — 先执行 `git status` 获取变更文件列表，以编号列表展示，让用户选择编号
> 6. **任何其他列表场景** — 一律以编号列表形式展示，让用户选择
>
> ❌ 错误做法：列出名称后说"请告诉我分支名称"
> ✅ 正确做法：列出编号选项后说"请选择编号"
