---
name: git-studio
description: 蒸馏 Android Studio Git 工具的全流程操作技能，100%覆盖完整 Git 工作流。当用户说"帮我提交一下"、"推一下代码"、"切个分支"、"合并一下"、"代码回退"、"拉一下最新代码"、"看看提交记录"、"谁改的这行"、"暂存一下"、"打个tag"、"代码冲突了"、"克隆这个仓库"、"建个新项目"、"搁置一下改动"等任何 Git 相关操作时触发。
---
...
> **凡是需要用户从多个选项中选择的场景，必须使用 `ask_user_question` 工具弹出可点击选项面板，让用户点击选择。**
>
> 适用场景：
> 1. **分支选择** — 先执行 `git branch -a` 获取分支列表，通过 `ask_user_question` 提供可点击的分支选项
> 2. **标签选择** — 先执行 `git tag` 获取标签列表，通过 `ask_user_question` 提供可点击的标签选项
> 3. **Stash 选择** — 先执行 `git stash list` 获取列表，通过 `ask_user_question` 提供可点击的 stash 选项
> 4. **提交选择** — 先执行 `git log --oneline` 获取提交列表，通过 `ask_user_question` 提供可点击的提交选项
> 5. **文件选择** — 先执行 `git status` 获取变更文件列表，通过 `ask_user_question` 提供可点击的文件选项
> 6. **操作模式/确认选择** — 如重置模式、恢复方式、删除确认等，通过 `ask_user_question` 提供可点击选项
> 7. **任何其他选择场景** — 一律使用 `ask_user_question` 工具
>
> ❌ 错误做法：在对话中列出编号表格，让用户输入编号
> ✅ 正确做法：使用 `ask_user_question` 工具弹出选项面板，用户点击即可
>
> 💡 仅在需要自由文本输入时（如输入新分支名称、提交信息、URL 等）才直接在对话中询问

## 何时使用

- 用户说"帮我提交"、"提交一下"、"commit 一下"、"提个代码"
- 用户说"推一下"、"push 上去"、"推送代码"、"发上去"
- 用户说"拉一下"、"更新一下代码"、"pull 最新的"、"同步一下"
- 用户说"切个分支"、"建个分支"、"换到 xx 分支"、"删掉这个分支"
- 用户说"合并一下"、"merge 进来"、"rebase 一下"、"cherry-pick 这个提交"
- 用户说"代码冲突了"、"冲突怎么解决"、"有冲突"
- 用户说"回退一下"、"撤销刚才的"、"代码还原"、"恢复到之前"
- 用户说"暂存一下"、"stash 起来"、"先存着"、"搁置一下"
- 用户说"看看日志"、"查看记录"、"谁改的"、"blame 一下"、"看看提交历史"
- 用户说"打个 tag"、"创建标签"、"发个版本"
- 用户说"克隆这个仓库"、"拉下来"、"git clone"
- 用户说"建个新项目"、"初始化仓库"、"git init"
- 用户提到任何 Git 相关操作或遇到 Git 问题
...
| 用户意图 | 路由目标 | 触发词（口语化） |
|----------|----------|------------------|
| 克隆/初始化 | [本文件 → 仓库初始化](#仓库初始化) | 克隆、拉下来、clone、建个新项目、初始化、git init |
| 提交代码 | [本文件 → 提交流程](#提交流程) | 提交一下、commit、提个代码、帮我提交 |
| 推送/拉取 | [本文件 → 远程操作](#远程操作) | 推一下、push上去、拉一下、更新代码、pull、同步一下、fetch |
| 变更列表 | [本文件 → Changelist](#changelist--变更列表) | 变更列表、changelist、分组管理改动 |
| 搁置变更 | [本文件 → Shelve](#shelve--搁置变更) | 搁置一下、shelve、先存着不提交 |
| 文件级操作 | [本文件 → 文件级操作](#文件级git操作) | 只提交这个文件、这个文件的历史、谁改的这个文件 |
| 分支操作 | [SKILL-branch.md](SKILL-branch.md) | 切个分支、建个分支、换到xx、删掉这个分支、分支列表 |
| 合并/变基 | [SKILL-merge.md](SKILL-merge.md) | 合并一下、merge进来、rebase一下、cherry-pick、冲突了 |
| 查看历史 | [SKILL-history.md](SKILL-history.md) | 看看日志、提交记录、谁改的、blame一下、diff、打个tag |
| 高级操作 | [SKILL-advanced.md](SKILL-advanced.md) | 暂存一下、stash、回退、撤销、还原、恢复、补丁、子模块、hooks、配置git |
...
> - **克隆选项**：通过 `ask_user_question` 提供可点击选项（question='选择克隆方式'）：
>   - **完整克隆**（默认）— 克隆全部历史
>   - **浅克隆** — `--depth 1`，仅最新提交（快速）
>   - **指定分支克隆** — `--branch <name>`
...
> 在当前目录初始化 Git 仓库：
> - 目录：`<current-dir>`
> - **初始选项**：通过 `ask_user_question` 提供可点击选项（question='选择初始化方式'）：
>   - **标准初始化** — 创建空仓库
>   - **初始化并关联远程** — 同时设置 remote origin
...
- 暂存区为空但有未暂存变更 → **🔄 询问用户：**
  ...
  > [列出变更文件]
  > 通过 `ask_user_question` 提供可点击选项（question='如何处理未暂存的变更？'）：
  > - **暂存全部** — `git add .`
  > - **选择文件暂存** — 列出文件让用户勾选
  > - **取消提交**
- 工作区干净 → 提示"没有需要提交的变更"
...
3. **🔄 询问用户：**
   ...
   > ```
   > 通过 `ask_user_question`（multiSelect=true）提供可点击的变更块选项（question='选择要暂存的变更块'），每个 Hunk 作为一个选项（label=Hunk 描述，description=行范围+变更摘要），用户点击勾选即可。另提供 **全部暂存** 选项。
...
> - **操作**：通过 `ask_user_question` 提供可点击选项（question='选择提交方式'）：
>   - **Commit and Push**（推荐）— 提交并推送到远程
>   - **Commit** — 仅提交到本地
>   - **修改提交信息** — 编辑后再提交
...
> ⚠️ 你要修改上次提交，请确认：
> - 上次提交：`<最近一条commit信息>`
> 通过 `ask_user_question` 提供可点击选项（question='Amend 上次提交的方式'）：
> - **Amend 并修改信息** — 重写提交信息
> - **Amend 保留信息** — 仅追加变更到上次提交
> - **取消** — 作为新提交
...
> 通过 `ask_user_question` 提供可点击选项（question='上次提交已推送，确认操作'）：
> - **继续 Amend** — 我了解风险，稍后 force push
> - **改为新提交** — 更安全的方式

### 阶段5：推送

**🔄 询问用户（如果阶段4未选择 Commit and Push）：**
> 通过 `ask_user_question` 提供可点击选项（question='提交成功！是否推送到远程？'）：
> - **推送**（推荐）
> - **暂不推送**
...
> 拉取远程更新，通过 `ask_user_question` 提供可点击选项（question='选择合并策略'）：
> - **Rebase**（推荐）— `git pull --rebase`，保持线性历史
> - **Merge** — `git pull`，保留合并提交
...
> ⚠️ 推送被拒绝：远程分支有新的提交。
> 通过 `ask_user_question` 提供可点击选项（question='推送被拒绝，选择同步方式'）：
> - **Rebase**（推荐）— 将本地提交变基到远程最新，保持线性历史
> - **Merge** — 创建合并提交，保留分支拓扑
> - **取消** — 暂不推送
...
> - 远程有 `<N>` 个你没有的提交将被覆盖
>
> 通过 `ask_user_question` 提供可点击选项（question='确认强制推送方式'）：
> - **Force Push (--force-with-lease)** — 安全版，若远程有他人新提交则拒绝
> - **Force Push (--force)** — 无条件覆盖（极危险）
> - **取消** — 不推送
...
> 📡 远程仓库管理：
> 先执行 `git remote -v` 展示当前远程列表，然后通过 `ask_user_question` 提供可点击的操作选项（question='选择远程仓库操作'）：
> - **添加远程仓库** — 新增一个远程源
> - **修改远程 URL** — 更改已有远程的地址
> - **重命名远程** — 修改远程名称
> - **删除远程** — 移除一个远程源
>
> 用户选择后，在对话中询问相应参数（远程名称、URL 等自由文本）。
...
2. **🔄 询问用户（对应 AS 的 Move to Another Changelist 对话框）：**
   > 当前变更文件：通过 `ask_user_question`（multiSelect=true）提供可点击的变更文件选项（question='选择第一个变更列表包含的文件'），每个文件作为一个选项（label=文件名，description=状态如 M/A/D）。
   > 用户点击勾选文件后，在对话中询问该变更列表的名称（自由文本，如 "用户登录功能"）。
   > 如需创建更多变更列表，重复上述步骤。未分配的文件将归入 "Default Changelist"。
...
> 搁置变更：先执行 `git status --short` 获取变更文件列表，通过 `ask_user_question`（multiSelect=true）提供可点击的文件选项（question='选择要搁置的文件（默认全部）'），每个文件作为一个选项（label=文件名，description=状态如 M/A/D）。另提供 **全部搁置** 选项。
> 用户点击选择后，在对话中询问搁置描述（自由文本，如 "WIP: 用户系统重构"）。
...
> 恢复搁置的变更：先执行 `git stash list` 获取列表，通过 `ask_user_question` 提供可点击的 stash 选项（question='选择要恢复的搁置记录'，label=搁置描述，description=时间+文件数）。
> 用户选择后，再通过 `ask_user_question` 提供操作选项（question='选择恢复方式'）：
> - **Pop** — 恢复并删除该搁置
> - **Apply** — 恢复但保留该搁置
> - **查看内容** — 先查看再决定
> - **删除** — 不恢复，直接删除
...
> 通过 `ask_user_question` 提供可点击选项（question='确认撤销文件修改？此操作不可恢复'）：
> - **确认撤销** — 恢复到上次提交的状态
> - **取消**
...
先执行 `git status --short` 获取变更文件列表，通过 `ask_user_question` 提供可点击的文件选项（label=文件名，description=状态）。

用户选择文件后，再通过 `ask_user_question` 提供查看方式选项：
- **简洁历史** — `git log --oneline`
- **含重命名追踪** — `git log --follow`
- **详细 diff** — `git log -p`
- **统计概览** — `git log --stat`
...
> - **目标文件**：`<file>`
> 通过 `ask_user_question` 提供可点击选项（question='选择标记方式'）：
> - **Assume Unchanged** — `git update-index --assume-unchanged`（轻量级，本地开发用）
> - **Skip Worktree** — `git update-index --skip-worktree`（更强，适合团队共享的配置文件）
> - **取消标记（Undo）** — 恢复 Git 对此文件变更的跟踪
> - **查看已标记文件列表** — 列出所有被标记的文件
...
> 将以下内容添加到 `.gitignore`？
> - **目标**：`<file-or-pattern>`
> - **添加到**：通过 `ask_user_question` 提供可点击选项（question='添加到哪个 .gitignore？'）：
>   - **项目根 `.gitignore`**（推荐）
...
先执行 `git branch -a` 获取分支列表，通过 `ask_user_question` 提供可点击的分支选项（label=分支名，description='将文件与此分支对比'）。
...
**Rebase 进行中：**
> ⚠️ 检测到 rebase 操作尚未完成。通过 `ask_user_question` 提供可点击选项（question='检测到未完成的 Rebase，如何处理？'）：
> - **继续 Rebase** — `git rebase --continue`，已解决冲突，继续
> - **跳过当前提交** — `git rebase --skip`，放弃此提交的变更
> - **放弃 Rebase** — `git rebase --abort`，回到 rebase 前的状态

**Merge 进行中：**
> ⚠️ 检测到 merge 操作尚未完成。通过 `ask_user_question` 提供可点击选项（question='检测到未完成的 Merge，如何处理？'）：
> - **完成 Merge** — 解决冲突后 `git add . && git commit`
> - **放弃 Merge** — `git merge --abort`，回到 merge 前的状态

**Cherry-pick 进行中：**
> ⚠️ 检测到 cherry-pick 操作尚未完成。通过 `ask_user_question` 提供可点击选项（question='检测到未完成的 Cherry-pick，如何处理？'）：
...
**🔄 当用户输入模糊的 Git 指令时，通过 `ask_user_question` 提供可点击的操作菜单（question='Git 操作'）：**
> - **Commit** — 提交变更
> - **Push** — 推送到远程
> - **Pull / Update** — 拉取远程更新
> - **Branches** — 分支管理
> - **Merge** — 合并分支
> - **Stash / Shelve** — 暂存/搁置变更
> - **Log / History** — 查看历史
> - **Rollback** — 回退/撤销
