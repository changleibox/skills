...
### ⚠️ API 调用方式（Python 脚本优先，curl 兜底）

**优先使用 Python 脚本**（`scripts/gitlab_api.py`），语法清晰、JSON 处理优雅、零额外依赖（仅用标准库 `urllib`）。

Python 脚本不可用时回退到 curl。

```bash
# 脚本路径（技能目录下）
SCRIPT_DIR="$(dirname "$(readlink -f "$0")")/scripts"  # 或直接使用绝对路径
GITLAB_API="python3 /path/to/skills/ishop-gitlab-issue-mr/scripts/gitlab_api.py"
```
...
**所有 python3/curl/git 命令执行时，必须设置 `has_risk: false`**，确保命令自动执行，不弹出确认框让用户点击。

只有以下不可逆操作设置 `has_risk: true`：
- 合并 MR（`gitlab_api.py merge_mr` 或 `merge_requests/<id>/merge`）
- 删除分支（`DELETE .../branches/<name>`）
- 关闭 Issue（`gitlab_api.py close_issue` 或 `state_event: close`）
- `git push -f`（强制推送）
...
**curl 方式（回退）：**
```bash
# Token 获取：优先 git config，备选环境变量
TOKEN=$(git config --global gitlab.token || echo $GITLAB_TOKEN)

curl -s -H "Private-Token: $TOKEN" ...
```

后续文档中同时给出 Python 和 curl 两种写法，实际执行时优先使用 Python 脚本。
