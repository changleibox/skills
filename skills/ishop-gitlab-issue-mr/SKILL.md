...
1. **curl 命令默认执行**：所有 curl API 调用直接执行，不询问用户许可
...
```bash
curl -s -H "Private-Token: $(git config --global gitlab.token)" \
  "https://git.graspishop.com/api/v4/projects?membership=true&simple=true&per_page=100" \
  | python3 -c "import sys,json; [print(f\"{p['path_with_namespace']} - {p['name']}\") for p in json.loads(sys.stdin.read())]"
```

使用 `ask_user_question` 列出项目让用户选择。

### Token 获取辅助函数

在所有 curl 命令中，Token 通过以下方式获取：

```bash
# 优先 git config，备选环境变量
TOKEN=$(git config --global gitlab.token || echo $GITLAB_TOKEN)
```

后续文档中 curl 命令统一使用 `$TOKEN` 变量，实际执行时替换为上述获取方式。
...
| curl 命令 | 直接执行 | 所有 API 调用默认执行，不询问用户许可 |
...
```bash
# 获取项目 ID（URL-encoded 项目路径）
PROJECT_PATH="igroup%2Fishop"  # 示例，实际从步骤2确定的项目获取
...
# 注意：GitLab API 创建 MR 时如果 source_branch 不存在，需要先创建分支
# 先创建分支
curl -s -X POST -H "Private-Token: $TOKEN" \
  -H "Content-Type: application/json" \
