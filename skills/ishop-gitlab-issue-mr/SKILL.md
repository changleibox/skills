...
**情况D：用户提供了 Issue/MR ID**

当用户直接提供 Issue ID（如"修改9745的bug"）或 MR ID 时，自动查找所属项目：

```bash
# 通过 Issue ID 查找项目
glab api issues/:iid --jq '.project_id'

# 或搜索 Issue
glab api issues --search "9745" --jq '.[] | "\(.project.path_with_namespace)|\(.iid)|\(.title)"'
```

找到项目后确认：

```
✅ Issue #9745 属于项目：igroup/ishop
